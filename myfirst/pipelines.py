...
class MmyfirstPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `myfirst`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM myfirst WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO myfirst (date, title, link, content)"
